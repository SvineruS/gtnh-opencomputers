...
function al1check(table, al1)
    ...
    return string.lower(al1) == string.lower(table.individual.active.species)
end
...
function al2check(table, al2)
    ...
    return string.lower(al2) == string.lower(table.individual.inactive.species)
end

function move(interface, side, table, number)
    -- table = ta[i] ONLY
...
function GET(purity, type, spec)
    -- RETURNS ta[i], ("pure","princess","common") or ("unpure","drone","meadows"), pure means al1 == al2
    local ti = int[1].getItemsInNetwork()
    local ta = int[1].getAvailableItems()
    local count = 0
    local tai
...
function GET1(type, spec1, spec2)
    -- RETURNS ta[i], gets only bees with BOTH spec genes present
    local ti = int[1].getItemsInNetwork()
    local ta = int[1].getAvailableItems()
    local count = 0
    local tai
...
function GET3(type, spec)
    -- RETURNS ta[i], counts bees of TYPE and at least 1 of genes SPEC
    local ti = int[1].getItemsInNetwork()
    local ta = int[1].getAvailableItems()
    local count = 0
    local tai
...
function breed(spec)
    ...
    local princess = nil
    local drone = nil
    local function getDrone(num)
        -- num is 1 for par1, 2 for par2, 3 for par1 AND par2
        if num == 2 then
